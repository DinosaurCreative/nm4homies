...
export const SideMenu = ({
  ...
  const hideMenuHandler = e => {
    if (e.target.id === "open") {
      ...
      // Разворачиваем popup при открытии меню
      if (setIsPopupMinimized) {
  ...
  return (
    <>
      <CSidebarToggler
        className={`side-menu side-menu__open-btn no-print  no-print`}
      ...
      <div
        className={`side-menu__background no-print ${menuHidden && "side-menu__background_hidden"}`}
        id="background"
      >
        <CSidebar className={`side-menu ${menuHidden && "side-menu__hidden"}`}>
          <CSidebarBrand>навигатор</CSidebarBrand>
          <CSidebarNav
            className={`side-menu__nav ${menuHidden && "side-menu__nav_hidden"}`}
          >
            <Link
              to="/"
              onClick={hideMenuHandler}
          ...
          <CButton
            color="dark"
            className={`side-menu__logout-btn ${menuHidden && "side-menu__logout-btn_hidden"}`}
